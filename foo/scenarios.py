...
def scenarioA3(ev3, left_motor, right_motor, front_motor, line_sensor, other_sensor, robot, obstacle_sensor):
  follow_line_until(other_sensor, line_sensor, robot, lambda x: stop_at_obstacle(x, obstacle_sensor, 185), side_factor=-1,PROPORTIONAL_GAIN=0.9 )   
  robot.turn(-15)
  front_motor.run_target(100, 80)
  robot.turn(45)
  front_motor.run_target(100, 0)
...
def scenarioA4(ev3, left_motor, right_motor, front_motor, line_sensor, other_sensor, robot, obstacle_sensor):
  print("Scenario A4")
  rotate_ccw_until(line_sensor, robot, lambda measurements: sharp_white_black_edge(measurements, reversed=True, thr=3), speed=20)
  follow_line_until(line_sensor, other_sensor, robot, lambda x: stop_at_obstacle(x, obstacle_sensor), side_factor=-1)
